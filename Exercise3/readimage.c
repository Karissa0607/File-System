...
void print_inode_directory(struct ext2_inode *inode_table, int inode_num) {
    struct ext2_inode *inode = &inode_table[inode_num - 1];
    for (int i = 0; i < inode->i_blocks/2; i++) {
	...
	while (length < EXT2_BLOCK_SIZE) {
	    ...
	    char type; 
	    unsigned char dir_type = entry->file_type; 
	    if (dir_type == EXT2_FT_REG_FILE) {
		type = 'f';
	    } else if (dir_type == EXT2_FT_DIR) {
		type = 'd';
	    } else if (dir_type == EXT2_FT_SYMLINK) {
		type = '1';
	    }
...
int main(int argc, char **argv) {
    ...
    print_inode_directory(inode_table, EXT2_ROOT_INO);
    for (int i = sb->s_first_ino; i < sb->s_inodes_count; i++) {
	int byte = i / 8;
	int bit = i % 8;
	if (inode_bitmap[byte] & (1 << bit)) {
	    if ((inode_table[i].i_mode & EXT2_S_IFDIR) != 0) {//only print directories
